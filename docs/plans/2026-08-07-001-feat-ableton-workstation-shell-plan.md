...
- KTD12. **Live-page transport/browser shortcuts live in one keymap module (`keymap.ts`) shared by the window handler and `?` cheat sheet.** Ableton Space (stop→start) vs Shift+Space (continue); Home/`.` to start; `L` loop; Mod+F focus filter (bare F stays synth); Esc dismisses overlays. Undo/redo/save deferred (no undo stack / session persistence). Typing contexts suppress shortcuts. (session-settled: user-directed follow-up.)
